...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    if not get_doc_shelf(doc_number):
...
def show_all_docs_info():
    return [show_document_info(current_document) for current_document in documents]
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc_shelf_number = add_new_doc(
                new_doc_number, new_doc_type,
                new_doc_owner_name, new_doc_shelf_number)
